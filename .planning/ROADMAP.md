...
- [ ] **Phase 7: HAPP Subscription Server** — встроенный socat-handler на 127.0.0.1:8080 + полный набор HAPP headers + revoke menu
...
**Success Criteria**:
1. `update_xray_core()` в update.sh: detect `uname -m` → MACHINE, читает latest tag из GitHub Releases API, сравнивает с `xray version`, скачивает `Xray-linux-${MACHINE}.zip` + `.dgst`, **обязательно** сверяет SHA-256 (формат `256= <hex>`), self-test нового binary через `version`, бэкапит старый в `xray.bak.$(date +%s)`, atomic `install -m 755`, `safe_restart_xray()` после замены, rollback бинарника на failure.
...
7. После Phase 5 сервер запущен на Xray-core ≥ 25.3 (нужен `xray vlessenc` для Phase 6).
...
**Depends on**: Phase 4 (safe_jq_write + run_migration + safe_restart обязательны для PQ-миграций), Phase 5 (Xray-core 25.3+ — `xray vlessenc` появился именно там)
...
- **Plan 6.1 — PQ key infrastructure**: REQ-A01, REQ-A02, REQ-A03. install.sh + migration `.mlkem_keys_generated` + константы файлов. Минимальный, но foundational для остальных планов фазы. **Checkpoint:** до написания парсера `xray vlessenc` — запустить команду на реальном Xray-core 25.3+ и зафиксировать точный stdout формат (см. RESEARCH §10).
...
**Risks**:
- **Live-disconnect при upgrade существующего профиля**: добавление parallel pq-inbound требует `safe_restart_xray()`, который reload-ит все клиенты. *Митигация:* предупреждение в кнопке "Upgrade" + 30-day grace на параллельный legacy + рекомендация делать upgrade вне peak-часов. На свежесозданных профилях рестарт неизбежен (это ожидаемо для апгрейда).
- **Profile JSON schema drift**: profile-файл теперь хранит ДВА UUID + два transport блока, что ломает любые внешние тулзы, парсящие profiles/*.json. *Митигация:* версионирование (`schema_version: 2` в profile JSON), чёткий чек в read-функциях, миграция per-profile при первом open.
...
**Goal**: Оператор включает один пункт меню — получает работающий HTTP subscription endpoint, по которому HAPP one-tap импортирует подписку с обеими vless:// (pq + legacy), полным набором HAPP headers и возможностью revoke токена.
...
**Requirements**: REQ-C01, REQ-C02, REQ-C03, REQ-C04, REQ-C05, REQ-C06, REQ-C07, REQ-C08, REQ-C09, REQ-C10, REQ-C11, REQ-C12, REQ-C13

**Success Criteria**:
1. `install_subscription_server()` функция в xrayebator heredoc-генерирует `/usr/local/bin/subhttp.sh` (handler), `/etc/systemd/system/xrayebator-sub.service` (unit) и `/usr/local/etc/xray/.happ_defaults.env` (configurable env). Опт-ин через пункт меню.
2. **Mode A (default)**: socat на `127.0.0.1:8080`. Порт **не** открывается в UFW. Доступ только через SSH-tunnel или локальный QR.
3. **Mode B (опциональный)**: отдельный пункт меню "Настроить публичный TLS subscription server" — устанавливает nginx + certbot (Let's Encrypt), требует домена, открывает 8443 в UFW (`ufw limit 8443/tcp`).
4. Каждый профиль получает 32-hex token (`openssl rand -hex 16`), хранится в profile JSON как `sub_token`. Migration `.subscription_tokens_2026` через `run_migration` догенерирует токены на existing профилях.
...
6. Response: HTTP 200, `content-type: application/json; charset=utf-8`, `content-disposition: attachment; filename="xrayebator-${token:0:8}.json"`. Body — формат B HAPP: line-separated `vless://` + inline `#header:` строки.
7. Полный набор HAPP headers (как HTTP-headers И как inline `#header:` дубль): `subscription-userinfo: upload=0; download=0; total=0; expire=4102444800` (placeholder v2.0), `profile-update-interval: 24` (hardcoded), `profile-title: base64:$ENCODED`, `support-url`, `profile-web-page-url`, `announcement: base64:$ENCODED` (если задан).
...
10. *(REVISED 2026-05-10)* Subscription для каждого профиля отдаёт ОДНУ vless:// ссылку (соответствующую transport профиля — PQ или legacy). Parallel legacy fallback DROPPED — REQ-A07 снят.
11. Меню "Управление подпиской": показать subscription URL с токеном, revoke (regenerate `sub_token` + restart subhttp), показать QR-код subscription URL.
12. Меню "Настройки HAPP": редактировать `.happ_defaults.env` (title/support_url/profile_web_page_url/announcement) через TUI. Изменения применяются без рестарта (subhttp source-ит env на каждый request).

**Plan structure suggestion**:
- **Plan 7.1 — Pure VLESS URL extraction + token migration**: REQ-C04, REQ-C10, REQ-C11. Foundational рефакторинг (pure function для re-use) + token-generation migration. Это unblock-ер для остальных планов.
- **Plan 7.2 — subhttp.sh + systemd unit + Mode A**: REQ-C01, REQ-C02, REQ-C05, REQ-C06, REQ-C07, REQ-C09. Сердцевина: heredoc-генерация handler, security-hardened systemd unit, strict regex routing, полный набор HAPP headers, default loopback-only mode.
- **Plan 7.3 — Mode B (TLS) + management menus**: REQ-C03, REQ-C08, REQ-C12, REQ-C13. Опциональный nginx+LE flow, UFW rate-limit, revoke menu, .happ_defaults.env TUI.

**Risks**:
- **socat per-request fork model плохо масштабируется** (HAPP-клиенты могут polling-ить каждый час): на 100+ профилей сервер захлебнётся CPU. *Митигация:* для Mode A это допустимо (loopback, одиночные запросы); для Mode B nginx стоит впереди и абсорбирует concurrency через UFW limit. В v2.1 можно переехать на Python http.server, если станет проблемой.
...
**Goal**: SNI list актуализирован под РФ-доноров 2026 (банки, логистика, маркетплейсы), оператор получает опциональное experimental-меню для testpre/testseed и полноценное меню обхода VPN (Steam, банки, gosuslugi, OZON, Yandex), а HAPP announcement редактируется одной кнопкой.

**Depends on**: Phase 7 (REQ-E04 — announcement читается subhttp.sh; bypass-routing использует safe-pattern из Phase 4)
...
4. Пункт меню "Установить announcement для HAPP": запрашивает текст, сохраняет в `/usr/local/etc/xray/announcement.txt`. subhttp.sh из Phase 7 читает этот файл и эмиттит как HAPP `announcement` header (base64).
...
**Plan structure suggestion**:
- **Plan 8.1 — SNI 2026 + probe-test + experimental Vision Seed + announcement**: REQ-E01, REQ-E02, REQ-E03, REQ-E04. Группа мелких независимых улучшений, можно сделать одним планом.
...
| HAPP subscription URL отдаёт корректные headers + одну vless:// строку (transport профиля) | Phase 7 (REQ-C06, C07, C11 REVISED 2026-05-10 — одна ссылка на профиль) — depends on Phase 6 (PQ inbound metadata в profile JSON) |
| HAPP-юзер импортирует подписку одним кликом, получает рабочее соединение | Phase 7 end-to-end (Mode A loopback по умолчанию + revoke menu + happ_defaults configurable) |
...
Все 7 milestone success criteria покрыты. Coverage: 44/44 requirements ✓.
